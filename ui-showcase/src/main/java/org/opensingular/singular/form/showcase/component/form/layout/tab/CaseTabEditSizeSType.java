 ...
 */
/*hidden*/@CaseItem(componentName = "Tabs", subCaseName = "Edit size", group = Group.LAYOUT,
/*hidden*/        resources = {@Resource(CaseLayoutPackage.class), @Resource(STypeInformacoes.class)})
@SInfoType(spackage = CaseLayoutPackage.class)
public class CaseTabEditSizeSType extends STypeComposite<SIComposite> {

    public STypeInformacoes informacoes;
    public STypeAddress endereco;

    @Override
    protected void onLoadType(@Nonnull TypeBuilder tb) {
        informacoes = this.addField("informacoes", STypeInformacoes.class);
        endereco = this.addField("endereco", STypeAddress.class);

        SViewTab tabbed = new SViewTab();
        tabbed.addTab(informacoes,"Informações");
        tabbed.addTab(endereco,"Endereço");
        //@destacar
